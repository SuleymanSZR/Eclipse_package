CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
      ...
      BEGIN OF total_status,
        num1 TYPE int4  VALUE '1000',
        num2 TYPE int4  VALUE '3000',
        num3 TYPE int4  VALUE '5000',
      END OF total_status.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    DATA:
      entity           TYPE STRUCTURE FOR CREATE zr_rap_ss_002,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap_ss_002 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap_ss_002_d FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF zr_rap_ss_002 IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF zr_rap_ss_002 IN LOCAL MODE
      ENTITY Travel
        UPDATE SET FIELDS
...
**************************************************************************
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE zr_rap_ss_002.
...
**************************************************************************
  METHOD copyTravel.
    DATA:
       travels       TYPE TABLE FOR CREATE zr_rap_ss_002\\travel.
    ...
    result = VALUE #( FOR travel IN travels
                      ( %tky                   = travel-%tky
                        ...
                        %action-deductDiscount = COND #( WHEN travel-OverallStatus = travel_status-open
                                                         THEN if_abap_behv=>fc-o-enabled ELSE if_abap_behv=>fc-o-disabled   )
                        %action-totalPrice     = COND #( WHEN travel-OverallStatus = travel_status-accepted
                                                         THEN if_abap_behv=>fc-o-enabled ELSE if_abap_behv=>fc-o-disabled   )
...
*    MODIFY ENTITIES OF zr_rap_ss_002 IN LOCAL MODE
*       ENTITY Travel
*       UPDATE FIELDS ( CurrencyCode TotalPrice BookingFee )
*       WITH VALUE #( FOR key IN keys ( %tky          = key-%tky
*                                        CurrencyCode = 'TRY'
*                                        TotalPrice   = total_status-num2
*                                        BookingFee   = total_status-num3  ) )
**       WITH VALUE #( FOR key IN keys ( %tky          = key-%tky
**                                        CurrencyCode = COND #( WHEN key-TravelID = 2
**                                                               THEN 'USD' ELSE 'TRY' )
**                                        TotalPrice   = COND #( WHEN key-TravelID = 10
**                                                               THEN total_status-num1 ELSE total_status-num3 )
**                                        BookingFee   = COND #( WHEN key-TravelID = 10
**                                                               THEN total_status-num2 ELSE total_status-num1 ) ) )
*    FAILED failed
*    REPORTED reported.
*
*    " read changed data for action result
...
****************************************************************************
    ...
    DATA lt_update TYPE TABLE FOR UPDATE zr_rap_ss_002.
